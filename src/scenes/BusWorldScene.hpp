...
void cBusWorldScene::Update()
{
    ...
    if(paKeys[GLFW_KEY_U])
        ENGINE_LOG("Current position: " << poCamera->GetPosition().x << ", " << poCamera->GetPosition().y << ", " << poCamera->GetPosition().z);
...
void cBusWorldScene::HandleKey(uint uiKeyCode, uint uiAction)
{
    ...
    if(uiAction == GLFW_PRESS && uiKeyCode == GLFW_KEY_1)
    {
        updatePenguins ? updatePenguins = false : updatePenguins = true;
    }
}
...
void cBusWorldScene::LoadObjects()
{
    ...
    for(int i = 0; i < 20; i++)
    {
        for(int j = 0; j < 20; j++)
        {
            ...
            pmpObjects[key] = new cPassenger(pmpMeshes["passenger"]);
            pmpObjects[key]->SetPosition(glm::vec3(-60.0f + j, 0.0f, -60.0f + i));
            entityGroup.AddEntity(dynamic_cast<cPassenger*>(pmpObjects[key]));
            dynamic_cast<cPassenger*>(pmpObjects[key])->SetTarget(glm::vec3(-10.0f, 0.0f, -10.0f));
        }
    }
