...
namespace JavaScriptEntityFunctions
{
    ...
    duk_ret_t AppendEntitySteeringForce(duk_context *poContext)
    {
        ...
        float fXCoord = duk_to_number(poContext, -2);
        float fYCoord = duk_to_number(poContext, -1);

        if(!isnan(fYCoord) && !isnan(fXCoord))
            poEntity->AppendSteeringForce(glm::vec2(fXCoord, fYCoord));
        else
            ENGINE_WARN("Behaviour script return NaN!");
